...
| 40 | [USB claims one interface and the app needs three](FINDING-usb-claims-one-interface-and-the-app-needs-three.md) | high for real hardware - over USB the app could speak the security-key protocol and nothing else. **Interface half fixed and MEASURED on a physical key**; all four claimed, each identified by usage page | ours | n/a |
...
| 45 | [the soft key firmware thread stopped during BLE work](FINDING-the-soft-key-firmware-thread-stopped-during-ble-work.md) | unknown — OBSERVED ONCE and not reproduced in three runs after; the vendor GATT path was proven innocent by logcat, which is the only reason three failing tests were not read as "the new BLE code is broken". Related to `#44` from the other side: there the thread lived on while the session desynced | **ours** | n/a |
